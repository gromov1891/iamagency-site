...
export default function IntensiveClient({ locale = "ru" }: { locale?: "ru" | "en" }) {
  ...
  return <div ref={motionRoot}>
    ...
    <section className={`${styles.canvas} ${styles.format}`} id="format">
      ...
      <div data-reveal className={styles.formatCard}><strong>{isEnglish ? <>Pre-recorded lessons<br />with detailed<br />instructions</> : <>Записанные лекции<br />с подробными<br />инструкциями</>}</strong><span>{isEnglish ? "complete them at your own pace within one working week" : "вы проходите в своем темпе за рабочую неделю"}</span></div>
      <div data-reveal className={styles.formatFacts}>{isEnglish ? <><p>Every lesson includes <b>a practical assignment,</b> to reinforce the topic</p><p><b>3 plans</b><br />with different levels of support<br />for these 5 days</p><p><b>no filler,</b><br />only what you can put into practice</p></> : <><p>К каждой лекции идет <b>практическое задание,</b> которое закрепляет тему</p><p><b>3 тарифа</b><br />разница – в поддержке<br />на эти 5 дней</p><p><b>никакой воды,</b><br />только то что реально нужно применить!</p></>}</div>
    ...
    <section className={`${styles.canvas} ${styles.tariffs}`} id="tariffs">
      ...
      <div data-reveal className={styles.tariffTable}>
        ...
        {tariffs.map(item => <button key={item.id} type="button" className={tariff === item.id ? styles.selected : ""} onClick={() => setTariff(item.id)}><h3>{item.id}<i aria-hidden="true" /></h3><p>{item.duration}{item.durationNote && <small>{item.durationNote}</small>}</p><p>{item.chat}{item.chatNote && <small>{item.chatNote}</small>}</p><p>{item.curator}{item.curatorNote && <small>{item.curatorNote}</small>}</p><p className={styles.price}>{item.price} ₽<small>{isEnglish ? "instead of" : "вместо"} {item.old} ₽</small></p></button>)}
      </div>
