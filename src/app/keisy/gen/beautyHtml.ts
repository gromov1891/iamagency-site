...
const ph = (
  ...
  if (_f) return `<img src="/blk/keisy/${_f}" alt="кейс I AM AGENCY - beauty" loading="lazy" style="position:absolute;left:${x}px;top:${y}px;width:${w}px;height:${h}px;${r ? `border-radius:${r};` : ""}object-fit:cover;" />`;
...
const avatar = (
  ...
  const _f = (IMGMAP as Record<string, string>)[node + "|" + (ref || "")];
  const inner = _f
    ? `<img src="/blk/keisy/${_f}" alt="кейс I AM AGENCY - beauty" loading="lazy" style="position:absolute;left:${x}px;top:${y}px;width:${d}px;height:${d}px;border-radius:50%;object-fit:cover;border:4px solid #FFF;box-sizing:border-box" />`
